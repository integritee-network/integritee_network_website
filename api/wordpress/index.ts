import { WP_API_URL } from '~/configs/app.config'
...
export const useWpApi = () => ({
  ...
  async getPosts({ limit = 10, cat }: { limit?: number; cat?: number | null }) {
  ...
  async getCategories() {
    return this.get<Category[]>(`${WP_API_URL}/categories?_fields=id,name,slug`)
  },
  ...
  async getUseCasesTaxonomies() {
    return this.get<UseCaseTag[]>(`${WP_API_URL}/use-cases-tag?_fields=id,name`)
  },
})
